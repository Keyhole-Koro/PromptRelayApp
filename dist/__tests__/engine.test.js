...
describe("engine", () => {
    ...
    async function startGameAndTurn() {
        setupGame();
        await engine.startGame();
        // After startGame: GAME_STARTED dispatched, TURN_STARTED dispatched
        // Add a prompt so tick has something to generate from
        const currentTurnPlayerId = engine.state.turn?.order[engine.state.turn.currentPlayerIndex];
        if (!currentTurnPlayerId) {
            throw new Error("expected current turn player id");
        }
        engine.appendPrompt(currentTurnPlayerId, "a cute cat");
    }
    ...
    it("(b) emits IMAGE_REQUESTED after 10s tick", async () => {
        ...
        const imageRequested = broadcasts.filter((b) => b.event.type === "IMAGE_REQUESTED");
    ...
    it("(d) triggers TURN_ENDED and final image generation after 30s", async () => {
        ...
        const finalImageReqs = broadcasts.filter((b) => b.event.type === "IMAGE_REQUESTED" && b.event.type === "IMAGE_REQUESTED" && b.event.isFinal);
    ...
    it("dispatches ERROR event when worker fails", async () => {
        ...
        const gameStarted = broadcasts.filter((b) => b.event.type === "GAME_STARTED");
        expect(gameStarted).toHaveLength(1);
        // Engine state should not be crashed
        expect(engine.state.phase).toBe("playing");
    });
    it("accepts prompt updates only from the active turn player", async () => {
        setupGame();
        await engine.startGame();
        ...
        if (!turn)
            return;
        const activePlayerId = turn.order[turn.currentPlayerIndex];
        const nonActivePlayerId = turn.order.find((id) => id !== activePlayerId);
        expect(nonActivePlayerId).toBeDefined();
        if (!nonActivePlayerId)
            return;
        const accepted = engine.appendPrompt(activePlayerId, "valid");
        const rejected = engine.appendPrompt(nonActivePlayerId, "invalid");
        expect(accepted).toBe(true);
        expect(rejected).toBe(false);
        expect(engine.state.prompts).toHaveLength(1);
        expect(engine.state.prompts[0].playerId).toBe(activePlayerId);
    });
    it("sets turn order as a permutation of all players on game start", async () => {
        setupGame();
        await engine.startGame();
        const turn = engine.state.turn;
        expect(turn).not.toBeNull();
        if (!turn)
            return;
        const expectedIds = engine.state.players.map((p) => p.id).sort();
        const actualIds = [...turn.order].sort();
        expect(actualIds).toEqual(expectedIds);
        expect(new Set(turn.order).size).toBe(engine.state.players.length);
